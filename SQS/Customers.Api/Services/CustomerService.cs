...
public class CustomerService : ICustomerService
{
    ...
    public CustomerService(ICustomerRepository customerRepository,
        ...
        ISQSMessanger sqsMessenger)
    {
        _customerRepository = customerRepository;
        _gitHubService = gitHubService;
        _sqsMessanger = sqsMessenger;
    }
    ...
    public async Task<bool> CreateAsync(Customer customer,CancellationToken cancellationToken)
    {
        ...
        if (response)
        {
          _ = await _sqsMessanger.SendMessageAsync<CustomerCreated>(customer.ToCustomerCreatedMessage(),cancellationToken);
    ...
    public async Task<bool> UpdateAsync(Customer customer,CancellationToken cancellationToken)
    {
        ...
        if (response)
        {
            _ = await _sqsMessanger.SendMessageAsync<CustomerUpdated>(customer.ToCustomerUpdatedMessage(), cancellationToken);
        }
    ...
    public async Task<bool> DeleteAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        if (response)
        {
            _ = _sqsMessanger.SendMessageAsync(new CustomerDeleted { Id = id}, cancellationToken);
        }
        return response;
    }
